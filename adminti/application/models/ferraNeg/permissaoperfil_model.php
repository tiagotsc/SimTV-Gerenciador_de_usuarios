...
class PermissaoPerfil_model extends CI_Model{
    ...
    public function inserePerfil(){
        ...
        $sql = "INSERT INTO sistema.perfil(nome_perfil, status_perfil, criador_perfil) VALUES(".$valorFormatado.", '".$this->input->post('status_perfil')."',".$this->session->userdata('cd').");";
        $this->db->query($sql);
        ...
        if($cd){
            ...
            foreach($this->input->post('permissao') as $perm){
                ...
                $sql = "INSERT INTO sistema.permissao_perfil (cd_permissao, cd_perfil) VALUES (".$perm.", ".$cd.");";
                $this->db->query($sql);
    ...
    public function atualizaPerfil($cd){
        ...
        $sql = "UPDATE sistema.perfil SET nome_perfil = ".$valorFormatado.", status_perfil = '".$this->input->post('status_perfil')."', atualizador_perfil = ".$this->session->userdata('cd').", data_atualizacao_perfil = '".date('Y-m-d h:i:s')."' WHERE cd_perfil = ".$cd;
        
        if($this->db->query($sql)){
            
            $sql = "DELETE FROM sistema.permissao_perfil WHERE cd_perfil = ".$cd;
            $this->db->query($sql);
	...
	public function perfil(){
		...
		return $this->db->get('sistema.perfil')->result();
	}
    ...
    public function permissoesDoPerfil($cd){
		...
		return $this->db->get('sistema.permissao_perfil')->result_array();
    ...
    */
	public function dadosPerfil($cd){
		...
		$this->db->where('cd_perfil', $cd);
		$funcionario = $this->db->get('sistema.perfil')->result_array(); # TRANSFORMA O RESULTADO EM ARRAY
    ...
    */
	public function camposPerfil(){
		...
		$campos = $this->db->get('sistema.perfil')->list_fields();
    ...
    public function psqPerfis($nome = null, $status = null, $pagina = null, $mostra_por_pagina = null){
        ...
        return $this->db->get('sistema.perfil', $mostra_por_pagina, $pagina)->result();
    }
    ...
    public function deletePerfil(){
        ...
        $sql = "DELETE FROM sistema.perfil WHERE cd_perfil = ".$this->input->post('apg_cd_perfil');
        $this->db->query($sql);
